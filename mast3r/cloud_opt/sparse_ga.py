...
class SparseGA():
    def __init__(self, img_paths, pairs_in, res_fine, anchors, canonical_paths=None):
        ...
        self.distortion = res_fine.get('distortion')  # Get distortion parameters if available
        self.pts3d_colors = []
    ...
    def get_distortion(self):
        return self.distortion if self.distortion is not None else torch.zeros(3, device=self.working_device)
...
def sparse_global_alignment(imgs, pairs_in, cache_path, model, subsample=8, desc_conf='desc_conf',
                            device='cuda', dtype=torch.float32, shared_intrinsics=False, distortion=False, **kw):
    """ Sparse alignment with MASt3R
        ...
        distortion: whether to optimize for radial distortion parameters
    """
    ...
    imgs, res_coarse, res_fine = sparse_scene_optimizer(
        ...
        shared_intrinsics=shared_intrinsics, distortion=distortion, cache_path=cache_path, device=device, dtype=dtype, **kw)
...
def sparse_scene_optimizer(imgs, subsample, imsizes, pps, base_focals, core_depth, anchors, corres, corres2d,
                           ...
                           distortion=False,  # Enable radial distortion
                           init={}, device='cuda', dtype=torch.float32,
                           matching_conf_thr=5., loss_dust3r_w=0.01,
                           odometry_data=None, odometry_weight=1.0,scale_weight=1.0,
                           verbose=True, dbg=()):
    ...
    # Add radial distortion parameters (shared across all cameras)
    if distortion:
        # Initialize with zeros (no distortion)
        dist_coeffs = nn.Parameter(torch.zeros(3, device=device, dtype=dtype))
    else:
        dist_coeffs = None
    ...
    def apply_radial_distortion(points, dist_coeffs):
        """Apply radial distortion to normalized image coordinates.
        
        Args:
            points: Tensor of shape (..., 2) containing normalized image coordinates
            dist_coeffs: Tensor of shape (3,) containing distortion coefficients [k1, k2, k3]
            
        Returns:
            Tensor of shape (..., 2) containing distorted image coordinates
        """
        # Calculate squared radius
        r_squared = torch.sum(points * points, dim=-1, keepdim=True)
        
        # Calculate distortion factor
        factor = 1.0 + dist_coeffs[0] * r_squared + dist_coeffs[1] * r_squared**2 + dist_coeffs[2] * r_squared**3
    ...
    def loss_physical_constraints(cam2w,loss_fn=gamma_loss(0.7)):
        """Computes loss to enforce physical constraints.
        1. camera height should be 0.561
        2. camera pitch should be 0
        
        Args:
            cam2w: Camera-to-world transformations in OpenCV coordinates
            loss_fn: Robust loss function
        """
        # Extract camera heights (y-coordinate in OpenCV is down, so use negative)
        pred_heights = -cam2w[:, 1, 3]  # -y is up in OpenCV, (N)
        target_heights = torch.full_like(pred_heights, 0.561)
        
        # In OpenCV coordinates, camera looks along positive z
        # Pitch is rotation around x-axis, which affects the y and z components
        # of the camera's forward direction (third column of R)
        
        # Camera forward direction (z-axis in camera coordinates)
        # For zero pitch, the y component of forward should be 0
        # (forward vector should be in the xz-plane)
        forward_y = cam2w[:, 1, 2] # (N)
        target_forward_y = torch.zeros_like(forward_y)
        
        pitch_loss = loss_fn(torch.cat([pred_heights, ]), 
                             torch.cat([target_heights, ])).mean()
        
        # Combine losses
        return pitch_loss

    def loss_odometry(cam2w, odometry_data, loss_fn=torch.nn.functional.huber_loss):
        """Computes odometry loss with proper coordinate transforms between
        odometry frame (x-forward, y-left) and OpenCV camera frame (x-right, y-down, z-forward).
        
        Args:
            cam2w: Camera-to-world transformations in OpenCV coordinates
            odometry_data: List of dicts with keys 'from', 'to', 'dx', 'dy', 'dtheta'
            loss_fn: Robust loss function
            
        Returns:
            Loss term from odometry constraints
        """
        preds = []
        tgts = []
        theta_weight = 1
        ...
        for odom in odometry_data:
            idx1, idx2 = odom["from"], odom["to"]
            
            # Skip if both cameras are frozen
            if init[imgs[idx1]].get('freeze') and init[imgs[idx2]].get('freeze'):
                continue
                
            # Get the world-to-camera transforms
            w2cam1 = inv(cam2w[idx1])
            
            # Get the relative transform between cameras in OpenCV coordinates
            cam2_to_cam1 = w2cam1 @ cam2w[idx2]
            
            # Extract translation in OpenCV coordinates
            t = cam2_to_cam1[:3, 3]
            
            # Convert to odometry coordinate system:
            # - Forward (odom x) is z in OpenCV
            # - Left (odom y) is -x in OpenCV
            x_pred = t[2]  # z is forward in OpenCV
            y_pred = -t[0]  # -x is left in OpenCV
            z_pred = -t[1] * z_weight  # -y is down in OpenCV
            
            # Extract rotation matrix
            R = cam2_to_cam1[:3, :3]
            
            # Extract yaw (rotation around negative y-axis in OpenCV)
            # In OpenCV, rotation from z to -x corresponds to yaw in odometry frame
            theta_pred = torch.atan2(R[0, 2], R[2, 2]) * theta_weight
            
            # Get ground truth deltas from odometry
            dx = torch.tensor(odom["dx"], device=device)
            dy = torch.tensor(odom["dy"], device=device)
            dz = torch.tensor(0, device=device) # assume no change in height
            dtheta = torch.tensor(odom["dtheta"], device=device) * theta_weight
            
            preds.append(torch.stack([x_pred, y_pred, z_pred, theta_pred]))
            tgts.append(torch.stack([dx, dy, dz, dtheta]).float())
            
        return loss_fn(torch.cat(preds), torch.cat(tgts),delta=0.2)

    def loss_scale(cam2w, odometry_data,loss_fn=gamma_loss(0.8)):
        """Computes loss to enforce metric scale from odometry data.
        
        Args:
            cam2w: Camera-to-world transformations in OpenCV coordinates
            odometry_data: List of dicts with odometry measurements
            
        Returns:
            Loss term that enforces metric scale
        """
        pred_dists = []
        tgt_dists = []
        ...
        for odom in odometry_data:
            ...
            # Get the actual distance in reconstruction
            recon_dist = torch.sqrt(x_pred**2 + y_pred**2)
            
            # Get the metric distance from odometry
            odom_dx, odom_dy = odom["dx"], odom["dy"]
            odom_dist = torch.tensor(np.linalg.norm([odom_dx, odom_dy])).to(device)
            
            # if odom_dist > 1e-5:  # Avoid division by zero
            #     # Compute scale error - should be 1.0 if scales match
            #     scale_ratio = recon_dist / odom_dist
            #     scale_errors.append(scale_ratio - 1.0)  # Error should be zero
            pred_dists.append(recon_dist)
            tgt_dists.append(odom_dist)
        
        # # Combine all scale errors using robust loss
        # scale_errors = torch.stack(scale_errors)
        # return torch.mean(scale_errors.square())
        return loss_fn(torch.stack(pred_dists), torch.stack(tgt_dists)).mean()
    ...
    def loss_2d(K, w2cam, pts3d, pix_loss):
        ...
        for img1, pix1_filtered, confs_filtered, cf_sum, cleaned_slices in cleaned_corres2d:
            ...
            if pts3d_in_img1 != []:
                ...
                proj_pts = reproj2d(proj_matrix[img1], pts3d_in_img1)
                
                # Apply radial distortion if enabled
                if distortion:
                    # Normalize coordinates to apply distortion
                    focal = K[img1, 0, 0]
                    pp = K[img1, 0:2, 2]
                    norm_pts = (proj_pts - pp) / focal
                    
                    # Apply distortion
                    dist_pts = apply_radial_distortion(norm_pts, dist_coeffs)
                    
                    # Convert back to pixel coordinates
                    proj_pts = dist_pts * focal + pp
                
                loss += confs_filtered @ pix_loss(pix1_filtered, proj_pts)
    ...
    def optimize_loop(loss_func, lr_base, niter, pix_loss, lr_end=0):
        ...
        with tqdm(total=niter) as bar:
            for iter in range(niter or 1):
                ...
                # Add odometry loss if available
                if odometry_data is not None:
                    # odom_loss = loss_scale(cam2w, odometry_data) * scale_weight

                    odom_loss = loss_odometry(cam2w, odometry_data) * odometry_weight
                    loss += odom_loss

                    # physical_loss = loss_physical_constraints(cam2w) * 1
                    # loss += physical_loss
        ...
        result = dict(intrinsics=K.detach(), cam2w=cam2w.detach(),
                    depthmaps=[d.detach() for d in depthmaps], pts3d=[p.detach() for p in pts3d])
        
        # Add distortion parameters to result if enabled
        if distortion:
            result['distortion'] = dist_coeffs.detach()
    ...
    if distortion:
        print('Final distortion coefficients = ', to_numpy(dist_coeffs))

    # for pose in res_fine['cam2w']:
    #     print(pose[:3, 3])
    # # get delta poses
    # delta_cam_poses = []
    # print(f'Delta poses:')
    # for i in range(len(res_fine['cam2w'])-1):
    #     inv_pose = inv(res_fine['cam2w'][i])
    #     delta_pose = inv_pose @ res_fine['cam2w'][i+1]
    #     delta_cam_poses.append(delta_pose)
    #     print(delta_pose[:3, 3])
